...
export default function DevelopersPage() {
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#0C1C14' }}>
      {/* Header */}
      <header style={{ background: '#0C1C14', borderBottom: '1px solid rgba(74,139,100,0.06)', padding: '16px 0' }}>
        <div style={{ maxWidth: '1100px', margin: '0 auto', padding: '0 24px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
            ...
            <span style={{ fontFamily: bg, fontSize: '11px', fontWeight: 700, color: '#4A8B64', letterSpacing: '0.1em', padding: '2px 8px', border: '1px solid rgba(74,139,100,0.2)', borderRadius: '4px' }}>DEVELOPERS</span>
          </div>
          <Link href="/register" style={{ fontFamily: bg, fontSize: '13px', fontWeight: 600, color: '#0C1C14', background: '#4A8B64', padding: '8px 20px', borderRadius: '8px', textDecoration: 'none' }}>Get API keys</Link>
        </div>
      </header>
      ...
      <div style={{ maxWidth: '1100px', margin: '0 auto', padding: '60px 24px 40px' }}>
        ...
        <p style={{ fontFamily: bg, fontSize: '14px', color: '#6B8A74', marginTop: '12px' }}>
          Base URL: <code style={{ fontFamily: mono, fontSize: '13px', color: '#4A8B64', background: 'rgba(74,139,100,0.08)', padding: '2px 8px', borderRadius: '4px' }}>https://api.carbonbridge.ae/v1</code>
        </p>
      </div>

      {/* Tabs */}
      <div style={{ maxWidth: '1100px', margin: '0 auto', padding: '0 24px' }}>
        <div style={{ display: 'flex', gap: '4px', borderBottom: '1px solid rgba(74,139,100,0.08)', marginBottom: '40px' }}>
          {(['overview', 'reference', 'pricing'] as const).map(t => (
            <button key={t} onClick={() => setTab(t)} style={{
              ...
              borderBottom: tab === t ? '2px solid #4A8B64' : '2px solid transparent',
            }}>
        ...
        {tab === 'overview' && (
          <div style={{ paddingBottom: '80px' }}>
            {/* How it works */}
            <div style={{ background: 'rgba(255,252,246,0.02)', border: '1px solid rgba(74,139,100,0.06)', borderRadius: '14px', padding: '32px', marginBottom: '40px' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '24px' }}>
                {[
                  ...
                  <div key={s.step}>
                    <div style={{ fontFamily: fr, fontSize: '28px', fontWeight: 300, color: '#4A8B64', opacity: 0.5, marginBottom: '8px' }}>{s.step}</div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '16px', marginBottom: '40px' }}>
              {USE_CASES.map(uc => (
                <div key={uc.title} style={{ background: 'rgba(255,252,246,0.02)', border: '1px solid rgba(74,139,100,0.06)', borderRadius: '14px', padding: '24px', display: 'flex', gap: '24px', flexWrap: 'wrap' }}>
            ...
            {/* Sandbox */}
            <div style={{ background: 'linear-gradient(135deg, rgba(27,58,45,0.3), rgba(74,139,100,0.05))', border: '1px solid rgba(74,139,100,0.12)', borderRadius: '14px', padding: '32px', textAlign: 'center' }}>
              ...
              <Link href="/register" style={{ fontFamily: bg, fontSize: '14px', fontWeight: 600, color: '#0C1C14', background: '#4A8B64', padding: '12px 28px', borderRadius: '10px', textDecoration: 'none', display: 'inline-block' }}>
                Get API keys →
              </Link>
            </div>
          </div>
        ...
        {tab === 'reference' && (
          <div style={{ paddingBottom: '80px' }}>
            ...
            <p style={{ fontFamily: bg, fontSize: '14px', color: '#8AAA92', marginBottom: '28px' }}>
              Authentication: <code style={{ fontFamily: mono, fontSize: '12px', color: '#4A8B64' }}>Authorization: Bearer cb_live_xxxx</code> · Sandbox keys: <code style={{ fontFamily: mono, fontSize: '12px', color: '#4A8B64' }}>cb_test_xxxx</code>
            </p>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
              {ENDPOINTS.map(ep => {
                const mc = METHOD_COLORS[ep.method] || METHOD_COLORS.GET;
                return (
                  <div key={ep.path} style={{ background: 'rgba(255,252,246,0.02)', border: '1px solid rgba(74,139,100,0.06)', borderRadius: '12px', padding: '20px 24px' }}>
        ...
        {tab === 'pricing' && (
          <div style={{ paddingBottom: '80px' }}>
            ...
            <div style={{ background: 'rgba(255,252,246,0.02)', border: '1px solid rgba(74,139,100,0.06)', borderRadius: '14px', padding: '28px' }}>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '20px', marginBottom: '24px' }}>
                {[
                  ...
                  <div key={p.label} style={{ padding: '16px', background: 'rgba(74,139,100,0.03)', borderRadius: '10px' }}>
