...
export default function SettingsPage() {
  ...
  const Btn=({children}:{children:React.ReactNode})=>(<button style={{fontFamily:bg,fontSize:'13px',fontWeight:600,color:'#1B3A2D',background:'#4A8B64',border:'none',padding:'10px 24px',borderRadius:'8px',cursor:'pointer'}}>{children}</button>);
  return (
        ...
        <div style={{padding:'32px 28px',maxWidth:'900px'}}>
          ...
          <div style={{display:'flex',gap:'24px'}}>
            ...
            <div style={{flex:1,maxWidth:'640px'}}>
              {tab==='profile'&&<Card title="Personal Information"><div style={{display:'flex',alignItems:'center',gap:'16px',marginBottom:'24px'}}><div style={{width:'64px',height:'64px',borderRadius:'50%',background:'linear-gradient(135deg,#1B3A2D,#2D6A4F)',display:'flex',alignItems:'center',justifyContent:'center'}}><span style={{fontFamily:bg,fontSize:'22px',fontWeight:700,color:'#4A8B64'}}>E</span></div><div><div style={{fontFamily:bg,fontSize:'14px',fontWeight:600,color:'#1A1714'}}>Upload Photo</div><div style={{fontFamily:bg,fontSize:'11px',color:'#8B8178'}}>JPG, PNG · Max 2MB</div></div></div><div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'0 16px'}}><Inp label="First Name" value="Khalid"/><Inp label="Last Name" value="Al Mansouri"/></div><Inp label="Email" value="k.almansouri@emiratesindustrial.ae"/><Inp label="Phone" value="+971 50 123 4567"/><Inp label="Job Title" value="Head of Sustainability"/><div style={{display:'flex',justifyContent:'flex-end',marginTop:'8px'}}><Btn>Save Changes</Btn></div></Card>}
