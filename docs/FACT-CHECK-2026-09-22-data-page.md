...
Scope: `src/app/data/page.tsx` (this file lives in `docs/`). Stacked on #8, which corrected most of the regulatory timeline; two rows are corrected again here (below). Tick each box before merging.
...
- [ ] Price indices: kept for the layout (Gary's call whether they stay), titled "(in development)", each card badged **Illustrative**; the sub-heading says they are placeholders, not market prices, and that real indices will be built from settled marketplace transactions once trading starts; the footnote says illustrative, not live or historical prices, not investment advice. The numbers themselves are unchanged.
...
- [ ] Monthly volume chart: retitled "Monthly Retirements (illustrative)" (the old title said trading volume, the old sub-heading said retirements), an **Illustrative** badge inside the card so a screenshot carries it, sub-heading "Illustrative layout, not registry data. Placeholder … figures in millions of tonnes." Series unchanged.
...
## Timeline rows corrected again (found in review of this PR)

- [ ] **ICVCM row.** Was "Jul 2025 · ICVCM CCP Labels begin · First batch of Verra and Gold Standard methodologies receive CCP approval. Labels now visible on VCU serial numbers." ICVCM announced the first CCP-approved methodologies on **6 June 2024**: seven methodologies for landfill gas and ODS destruction (ACR, Climate Action Reserve, and CDM ACM0001 / AMS-III.G, "used by Verra and Gold Standard"), about 27 million credits eligible. Row now "Jun 2024 · First ICVCM CCP labels approved" with that detail. Source: https://icvcm.org/integrity-council-announces-first-high-integrity-ccp-labelled-carbon-credits-as-assessments-continue/
- [ ] **CBAM row.** The 50-tonne de minimis does not apply to electricity and hydrogen (as the `/compliance/eu-cbam` page already says, Regulation (EU) 2025/2083). Row now: "cement, iron and steel, aluminium and fertilisers above the 50-tonne annual de minimis, and electricity and hydrogen in any quantity".
- [ ] Timeline sub-heading "Key compliance milestones driving carbon credit demand" → "Key compliance and market-integrity milestones" (the VCMI and SBTi rows say they are not compliance requirements).

## Not changed (flagged)

- "Avg. price/tCO₂e" on the volume-by-type bars is a simple mean of listing prices, not volume-weighted. Left as is.
